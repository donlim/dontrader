...
def generate_signal(indicators, weights=None):
    if weights is None:
    ...
    raw_scores = {
        ...
        "SMA_DIFF": sma_signal(indicators.get("SMA_DIFF")),  # keep this if useful
        ...
        # --- Support / Resistance ---
        "DONCHIAN": donchian_signal(indicators.get("PRICE"), indicators.get("DONCHIAN_UPPER"), indicators.get("DONCHIAN_LOWER")),
        "DONCHIAN_WIDTH": donchian_width_signal(indicators.get("DONCHIAN_WIDTH")),
        "SUPPORT_RESISTANCE": support_resistance_signal(indicators.get("SUPPORT_LEVEL"), indicators.get("RESISTANCE_LEVEL")),
        ...
        # --- Other ---
        "PARABOLIC_SAR": parabolic_sar_signal(indicators.get("PARABOLIC_SAR"), indicators.get("PRICE")),
    }

    # === Normalize using tanh
        # === Normalize using tanh
    normalized_scores = {
        key: np.tanh(score) if score is not None else 0.0
    ...
    for key, score in normalized_scores.items():
        # If score is a Series (due to bad input), extract last scalar value
        if isinstance(score, pd.Series):
            score = score.iloc[-1]
        elif isinstance(score, np.ndarray):
            score = score[-1]  # fallback
    ...
    # Safety: ensure final_score is scalar
    if isinstance(final_score, (pd.Series, np.ndarray)):
        final_score = final_score.iloc[-1] if hasattr(final_score, 'iloc') else final_score[-1]
    ...
    if not isinstance(final_score, float):
        print(f"[ERROR] final_score type = {type(final_score)}; value = {final_score}")
    ...
    top_indicators = sorted(
        ...
        key=lambda x: abs(to_scalar_safe(x[1]) * weights.get(x[0], 0)),
        reverse=True
    )[:5]

    return signal, category_subscores, final_score, normalized_scores, meta_confidence, mode, top_indicators
